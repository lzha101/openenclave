...
static unsigned char tcps_oid[] = {0x67, 0x81, 0x05, 0x05, 0x04, 0x02};
    ...
    oe_report_t* parsed_report)
{
    ...
    if (chain->next == NULL ||           // loader
        chain->next->next == NULL ||     // spl
        chain->next->next->next == NULL) // root
    {
        result = OE_FAILURE;
        goto Cleanup;
    }
    ...
    result = get_claims(
        chain->next,
        pidentity->loader_signer_id,
        sizeof(pidentity->loader_signer_id),
        pidentity->loader_unique_id,
        sizeof(pidentity->loader_unique_id),
        NULL,
        0);
    if (result != OE_OK)
    {
