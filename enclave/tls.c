...
// mbedtls_x509write_cert* x509cert,// output cert
// TODO: need to replace with some message here
//int ret = 0;
                                ...
                                size_t user_data_size)
{
    ...
    size_t remote_report_buf_size = 0;
    mbedtls_mpi serial;
    OE_SHA256 sha256 = {0};
    mbedtls_x509write_cert x509cert = { 0 };
    mbedtls_pk_context* subject_key = NULL; // subject's public key
    int ret = 0;
    int bytes_written = 0;
    ...
    ret = mbedtls_pk_parse_key(subject_key, (const unsigned char *)key,
                              key_size, NULL, 0);
    if (ret)
    {
    ...
    ret = mbedtls_x509write_crt_set_subject_name(&x509cert,
                                                 "CN=Open Encalve SDK,O=OESDK TLS,C=UK");
    if (ret)
    {
        OE_TRACE_ERROR("Failed with 0x%x\n", ret);
        goto done;
    ...
    //assert(ret == 0);
    ret = mbedtls_x509write_crt_set_issuer_name(&x509cert,
                                                "CN=Open Encalve SDK,O=OESDK TLS,C=UK");
    //assert(ret == 0);
