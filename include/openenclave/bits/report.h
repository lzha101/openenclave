...
typedef struct _oe_identity
{
    ...
    /** The unique ID for the TA loader.
     * For CyReS TAs, this is a combined hash of PPA image OP TEE image. */
    uint8_t loader_unique_id[OE_UNIQUE_ID_SIZE];

    /** The signer ID for the TA loader.
     * For CyReS TAs, this is a hash of the public key that signed the PPA and OP TEE images. */
    uint8_t loader_signer_id[OE_SIGNER_ID_SIZE];
